...
async def sign_to_text(req: SignToText):
    ...
    try:
        ...
        translator = deepl.Translator(os.getenv("DEEPL_API_KEY"))
        text_info = translator.translate_text(text, source_lang = "en", target_lang="en-us" if req.trg == "en" else req.trg)
...
async def audio_to_text(req: AudioToText):
    ...
    translator = deepl.Translator(os.getenv("DEEPL_API_KEY"))
    text_info = translator.translate_text(result["text"], source_lang = req.src, target_lang="en-us" if req.trg == "en" else req.trg)
    return {"result": str(text_info)}
